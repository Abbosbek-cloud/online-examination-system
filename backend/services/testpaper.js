...
let getSingletest = async (req, res, next) => {
  ...
  let testpaper = await TestPaperModel.find(
    { _id: id, status: 1 },
...
let getAlltests = async (req, res, next) => {
  if (req.user.type === "TRAINER") {
    var title = req.body.title;
    let testpaper = await TestPaperModel.find(
      { createdBy: req.user._id, status: 1 },
      { status: 0 }
    )
      .populate("questions", "body")
      .populate({
...
let basicTestdetails = async (req, res, next) => {
  if (req.user.type === "TRAINER") {
    let testid = req.body.id;
    let tests = await TestPaperModel.findById(testid, { questions: 0 })
      .populate("createdBy", "name")
      .populate("subjects", "topic")
      .exec();
    if (tests) {
...
let getTestquestions = async (req, res, next) => {
  if (req.user.type === "TRAINER") {
    ...
    let getTestquestions = await TestPaperModel.findById(testid, {
      type: 0,
      title: 0,
      subjects: 0,
      duration: 0,
...
let getCandidateDetails = async (req, res, next) => {
  if (req.user.type === "TRAINER") {
    ...
    let getCandidateDetails = await ResultModel.find(
      { testid: testid },
      { score: 1, userid: 1 }
    )
      .populate("userid")
...
let MaxMarks = (testid) => {
  return new Promise(async (resolve, reject) => {
    let Ma = await TestPaperModel.findOne({ _id: testid }, { questions: 1 })
      .populate({
        path: "questions",
        model: QuestionModel,
        select: { weightage: 1 },
